...
class TestDiscordTransport:
    ...
    def test_audio_conversion():
        ...
        discord_samples = np.random.randint(-32768, 32767, DISCORD_FRAME_BYTES // 2, dtype=np.int16)
        discord_audio = discord_samples.tobytes()
        
        # Test Discord -> Pipeline conversion
        processed_audio = resample_discord_to_pipeline(discord_audio)
        assert len(processed_audio) > 0
        
        # Should be downsampled by 3x and converted to mono int16
        actual_samples = len(processed_audio) // 2  # int16 = 2 bytes
        
        print(f"  Discord frame: {len(discord_audio)} bytes")
        print(f"  Processed frame: {len(processed_audio)} bytes ({actual_samples} samples)")
        
        # Test Pipeline -> Discord conversion (16kHz)
        back_to_discord = resample_pipeline_to_discord(processed_audio, PIPELINE_SAMPLE_RATE)
        ...
        tts_samples = np.random.randint(-16000, 16000, 480, dtype=np.int16)  # 20ms at 24kHz
        tts_audio = tts_samples.tobytes()
        discord_from_tts = resample_pipeline_to_discord(tts_audio, 24000)
    ...
    def test_audio_buffer():
        """Test placeholder — AudioBuffer is in v1, not v2."""
        print("Testing audio buffer VAD... (skipped — v1 only)")
        print("  ✅ Audio buffer tests skipped")
    ...
    def test_audio_source():
        ...
        # Feed some audio
        test_audio = np.random.randint(-1000, 1000, DISCORD_FRAME_BYTES // 2, dtype=np.int16).tobytes()
        source.feed(test_audio)
